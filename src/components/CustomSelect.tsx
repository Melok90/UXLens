...
export default function CustomSelect({
  ...
  const pyClass = size === 'md' ? 'py-2.5 sm:py-3 text-sm' : 'py-1.5 text-xs';

  return (
    <div className={`relative ${size === 'sm' ? 'min-w-[150px]' : 'w-full'} ${isOpen ? 'z-30' : ''} ${className}`} ref={containerRef}>
      <button
        type="button"
          ...
          >
            {options.map((option) => (
              <button
                key={option.value}
                type="button"
                className={`w-full text-left px-3.5 ${size === 'md' ? 'py-2.5 text-sm' : 'py-1.5 text-xs'} transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800/80 cursor-pointer ${
                  value === option.value
